
<div class="sidenav">

<div class="sidenav-top sidenav-mq-mobile-only">
  <a href="/contact-us" class="sidebar-button">Contact Us</a>

  <div class="sidenav-service-container-bespoke-software">
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-laptop"></span>
      <span class="sidenav-service-title-small">Bespoke</span>
      Software</a>

    <ul>
      <li><a href="#">Bespoke CRM</a></li>
      <li><a href="#">Business Automation</a></li>
      <li><a href="#">Software Integrations</a></li>
      <li><a href="#">Mobile App Development</a></li>
      <li><a href="#">Bespoke Databases</a></li>
      <li><a href="#">Sharepoint Development</a></li>
      <li><a href="#">Operational Systems</a></li>
      <li><a href="#">Business Central Implementation</a></li>
      <li><a href="#">Internet Of Things (IOT) Software</a></li>
      <li><a href="#">Intranet Development</a></li>
      <li><a href="#">Customer Portal Development</a></li>
      <li><a href="#">Reporting Hub</a></li>
      <li><a href="#">SAP S/4HANA Management</a></li>
    </ul>
  </div>

  <div class="sidenav-service-container-it-support">
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-display"></span>
      <span class="sidenav-service-title-small">IT</span>
      Support</a>

    <ul>
      <li><a href="#">Managed IT Support</a></li>
      <li><a href="#">Business IT Support</a></li>
      <li><a href="#">Office 365 For Business</a></li>
      <li><a href="#">IT Consultancy</a></li>
      <li><a href="#">Cloud Service Provider</a></li>
      <li><a href="#">Data Backup &amp; Disaster Recovery</a></li>
    </ul>
  </div>
  
  <div class="sidenav-service-container-digital-marketing">
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-bars"></span>
      <span class="sidenav-service-title-small">Digital</span>
      Marketing</a>
    
    <ul>
      <li><a href="#">Search Engine Optimisation (SEO)</a></li>
      <li><a href="#">Pay Per Click Advertising (PPC)</a></li>
      <li><a href="#">Conversion Rate Optimisation (CRO)</a></li>
      <li><a href="#">Email Marketing</a></li>
      <li><a href="#">Social Media Marketing</a></li>
      <li><a href="#">Content Marketing</a></li>
    </ul>
  </div>

  <div class="sidenav-service-container-telecoms-services"> 
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-phone_in_talk"></span>
      <span class="sidenav-service-title-small">Telecoms</span>
      Services</a>

    <ul>
      <li><a href="#">Business Mobile</a></li>
      <li><a href="#">Hosted VOIP Provider</a></li>
      <li><a href="#">Business VOIP Provider</a></li>
      <li><a href="#">Business Broadband</a></li>
      <li><a href="#">Leased Lines Provider</a></li>
      <li><a href="#">3CX Systems</a></li>
    </ul>
  </div>

  <div class="sidenav-service-container-web-design">
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-embed"></span>
      <span class="sidenav-service-title-small">Web</span>
      Design</a>

    <ul>
      <li><a href="#">Bespoke Website Design</a></li>
      <li><a href="#">Ecommerce Website Design</a></li>
      <li><a href="#">Pay Monthly Websites</a></li>
      <li><a href="#">Branding &amp; Design</a></li>
      <li><a href="#">Mobile App Development</a></li>
      <li><a href="#">Web Hosting</a></li>
    </ul>
  </div>

  <div class="sidenav-service-container-cyber-security">
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-security"></span>
      <span class="sidenav-service-title-small">Cyber</span>
      Security</a>

    <ul>
      <li><a href="#">Cyber Security Assessment</a></li>
      <li><a href="#">Cyber Security Management</a></li>
      <li><a href="#">Cyber Penetration Testing</a></li>
      <li><a href="#">Cyber Essentials Certification</a></li>
      <li><a href="#">PCI Compliance</a></li>
      <li><a href="#">Hacking Prevention</a></li>
    </ul>
  </div>

  <div class="sidenav-service-container-developer-training">
    <a href="#" class="sidebar-button sidenav-service-title">
      <span class="sidenav-service-icon icon-school"></span>
      <span class="sidenav-service-title-small">Developer</span>
      Course</a>

    <ul>
      <li><a href="#">Train for a Career in Tech</a></li>
      <li><a href="#">Skills Bootcamp</a></li>
      <li><a href="#">Scion Scheme Frequently Asked Questions</a></li>
      <li><a href="#">Scion Collaborators</a></li>
    </ul>
  </div>

</div>

<div class="sidenav-bottom">

  <div class="sidenav-bottom-container sidenav-mq-desktop-only"> 
    <a href="#" class="sidenav-bottom-header">
      <span>Services</span></a>

    <ul>
      <li><a href="#">Bespoke Software</a></li>
      <li><a href="#">IT Support</a></li>
      <li><a href="#">Digital Marketing</a></li>
      <li><a href="#">Telecoms Services</a></li>
      <li><a href="#">Web Design</a></li>
      <li><a href="#">Cyber Security</a></li>
    </ul>
  </div>

  <div class="sidenav-bottom-container">
    <a href="#" class="sidenav-bottom-header">
      <span>Our Work</span></a>
    
    <ul>
      <li><a href="#">Case Studies</a></li>
      <li><a href="#">Our Digital Marketing Clients</a></li>
      <li><a href="#">Our Website Clients</a></li>
      <li><a href="#">Our IT Clients</a></li>
      <li><a href="#">Our Bespoke Software Clients</a></li>
      <li><a href="#">Our Telecoms Clients</a></li>
    </ul>
  </div>

  <div class="sidenav-bottom-container"> 
    <a href="#" class="sidenav-bottom-header">
      <span>Our Knowledge</span></a>

    <ul>
      <li><a href="#">Technologies</a></li>
      <li><a href="#">Industries</a></li>
      <li><a href="#">News</a></li>
      <li><a href="#">Insights</a></li>
    </ul>
  </div>

  <div class="sidenav-bottom-container">
    <a href="#" class="sidenav-bottom-header">
      <span>Training</span></a>

    <ul>
      <li><a href="#">Train for a Career in Tech</a></li>
      <li><a href="#">Skills Bootcamp</a></li>
      <li><a href="#">SCS Frequently Asked Questions</a></li>
      <li><a href="#">Scion Collaborators</a></li>
    </ul>
  </div>

  <div class="sidenav-bottom-container">
    <a href="#" class="sidenav-bottom-header">
      <span>Our Company</span></a>

    <ul>
      <li><a href="#">Why Choose Us?</a></li>
      <li><a href="#">Our Culture</a></li>
      <li><a href="#">Our Team</a></li>
      <li><a href="#">Our Careers</a></li>
      <li><a href="#">Our Benefits</a></li>
      <li><a href="#">Our Accreditations</a></li>
    </ul>
  </div>

  <div class="sidenav-bottom-container">
    <a href="/contact-us" class="sidenav-bottom-header">
      <span>Contact Us</span></a>

    <ul>
      <li><a href="#">Cambridge Office</a></li>
      <li><a href="#">Wymondham Office</a></li>
      <li><a href="#">Great Yarmouth Office</a></li>
    </ul>
  </div>

  <div class="sidenav-bottom-container sidenav-mq-mobile-only">
    <a href="#" class="sidenav-bottom-header">
      <span>Support</span></a>
  </div>

</div>

</div>

<div class="body-content-cover"></div>